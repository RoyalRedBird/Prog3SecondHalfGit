...
public class GunshipController : MonoBehaviour
{
    ...
    private void FireCannonball(Vector3 direction, Transform cannon)
    {
        ...
        GameObject newBall = Instantiate(cannonballPrefab, cannon.transform.position, cannon.rotation);

        newBall.GetComponent<Rigidbody2D>().AddForce(direction * cannonballForce, ForceMode2D.Impulse);
